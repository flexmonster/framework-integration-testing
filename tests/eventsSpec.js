describe("testing events page", () => {
  ...
  it("Check link to docs", (browser) => {
    this.pivotContainer.section.description.expect.element("@toFMEventsLink").to.be.visible;
    this.pivotContainer.section.description.expect.element("@toFMEventsLink").text.to.be.equal("Flexmonster event");
    this.pivotContainer.section.description.expect
      .element("@toFMEventsLink")
  ...
  it("Check toggle(disable)", (client) => {
    this.pivotContainer.section.toggle.expect.element("@checkbox").to.be.selected;
    client.waitForElementVisible("#eventsToggle").click("#eventsToggle label");
  ...
  it("Check events output", (client) => {
    ...
    browser.pause(1000);
    this.pivotContainer.expect.section("@eventsOutput").to.be.visible;
    this.pivotContainer.section.eventsOutput.expect.element("@firstLine").to.be.visible;
    this.pivotContainer.section.clearOutput.expect.element("@clearOutputButton").to.be.visible;
  });
